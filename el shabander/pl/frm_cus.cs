    ...
    gridControl1.DataSource = dbContext.tb_cus.Local.ToBindingList();
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // جلب بيانات العميل للتحقق من total_price
                tB_cus = db.tb_cus.FirstOrDefault(x => x.id == id);

                if (tB_cus != null)
                {
                    // التحقق إذا كان total_price أكبر من 0
                    if (tB_cus.total_price > 0)
                    {
                        main frm_supp_Add = (main)Application.OpenForms["main"];

                        string notificationMessage = $"لقد حاول :  {frm_supp_Add.la_username.Text}   حذف العميل : {tB_cus.cus_name}  من البيانات ";
                        ...
                        db.SaveChanges();
                        MessageBox.Show("لا يمكن حذف هذا العميل لأنه لديه باقي حساب", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    ...
                    if (rs == DialogResult.Yes)
                    {
                        ...
                        string notificationMessage = $"تم حذف العميل : {tB_cus.cus_name} عن طريق : {frm_supp_Add.la_username.Text}  ";
            ...
            catch
            {
                ...
                dialog.txt_capthion.Text = "لا يوجد عميل لحذفه";
                dialog.Show();
            }
        }
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
           ...
           tB_cus= db.tb_cus.Where(x => x.id == id).FirstOrDefault();
            frm_supp_Add.edt_name.Text = tB_cus.cus_name.ToString();
            frm_supp_Add.edt_name_old.Text = tB_cus.cus_name.ToString();
            frm_supp_Add.edt_phone.Text = tB_cus.cus_phone.ToString();
            frm_supp_Add.edt_email.Text = tB_cus.cus_email.ToString();
            methods.by = tB_cus.cus_image;
            frm_supp_Add.edt_sales.ReadOnly = true;
            frm_supp_Add.edt_sales.Text=tB_cus.total_price.ToString();
            frm_supp_Add.edt_max.Text = tB_cus.cus_max.ToString();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            gridControl1.DataSource = db.tb_cus.Where(x => x.cus_name.Contains(_search) || x.cus_phone.Contains(_search) || x.cus_email.Contains(_search) ).ToList();
        }

        private void btn_spend_Click(object sender, EventArgs e)
        {
            pl.frm_spend_cus frm_supp_Add = new frm_spend_cus();
            //frm_supp_Add.id = 0;
           // frm_supp_Add.btn_add.Text = "اضافة";
            frm_supp_Add.Show();
        }
        public string GetHtmlFromTileView()
        {
            ...
            var data = db.tb_cus.ToList(); // أو استبدال tb_cus بالجدول الذي تحتاجه
            ...
            html += "<tr><th>الحساب المتبقي</th><th>العنوان</th><th>رقم الهاتف</th><th>اسم العميل </th></tr>";
            ...
            foreach (var item in data)
            {
                ...
                html += $"<td>{item.cus_email}</td>";
                html += $"<td>{item.cus_phone}</td>";
                html += $"<td>{item.cus_name}</td>";
                html += "</tr>";
            }
        ...
        private void tileView1_ItemCustomize(object sender, TileViewItemCustomizeEventArgs e)
        {
            ...
            var currentRow = view.GetRow(e.RowHandle) as tb_cus;
            if (currentRow == null)
                return;

            // الحصول على قيم total_price و supp_max
            double totalPrice = Convert.ToDouble(currentRow.total_price);
            double suppMax = Convert.ToDouble(currentRow.cus_max);
