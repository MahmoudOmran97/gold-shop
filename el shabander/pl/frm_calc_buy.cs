...
{
    public partial class frm_calc_buy : Form
    {
       ...
       tb_stuk tb_stuk = new tb_stuk();
        ...
        public void FetchAndCombineData()
        {
            ...
            var sells = db.tb_pur
                .ToList() // ← نحول الاستعلام لـ LINQ to Objects
                .Where(x => x.pur_dat.HasValue &&
                            x.pur_dat.Value.Date >= fromDate &&
                            x.pur_dat.Value.Date <= toDate)
                .Select(x => new
                {
                    cus_name = x.pur_supp,
                    Date = x.pur_dat,
            ...
            frm_report_view13 reportForm = new frm_report_view13();
