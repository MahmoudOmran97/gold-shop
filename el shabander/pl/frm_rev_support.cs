...
namespace el_shabander.pl
{
    public partial class frm_rev_support : Form
    {
        ...
        public frm_rev_support()
        {
            ...
            dbContext.tb_rev_supp.LoadAsync().ContinueWith(loadTask =>
            {
                // Bind data to control when loading complete
                gridControl1.DataSource = dbContext.tb_rev_supp.Local.ToBindingList();
        ...
        private void updata_data()
        {
          //  db = new gold_shopEntities();
            db = new gold_shopEntities();
            List<tb_rev_supp> data = db.tb_rev_supp.ToList(); // جلب البيانات
            data.Reverse();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            List<tb_rev_supp> data = db.tb_rev_supp.Where(x => x.name_supp.Contains(_search)).ToList();
            data.Reverse();
        ...
        {
            tb_supp = db.tb_supp.Where(x => x.supp_name == edt_name.Text).FirstOrDefault();
            text_mony.Text = tb_supp.total_price.ToString();
            sumsaler();
        }
        ...
        private void btn_add_Click_1(object sender, EventArgs e)
        {
            ...
            {
                sumsaler();
                tb_Rev_supp.name_supp = edt_name.Text;
                tb_Rev_supp.name_type = edt_type.Text;
                tb_Rev_supp.qt = Convert.ToDouble(text_qt.Text);
                tb_Rev_supp.price = stay;
                tb_Rev_supp.qt_stuk = Convert.ToDouble(edt_countsafe.Text);
                tb_Rev_supp.date = Convert.ToDateTime(edt_date.Text);
                tb_Rev_supp.mony_cust = Convert.ToDouble(text_mony.Text);
                tb_Rev_supp.stay_mony = Convert.ToDouble(text_stay.Text);
                ...
                if (frm_supp_Add != null)
                {
                    ...
                    {
                        tb_Rev_supp.namecasher = frm_supp_Add.la_username.Text;
                    }
                }
                if (small.Checked)
                {
                    tb_Rev_supp.unit = "صغرى";
                }else if (big.Checked)
                {
                    tb_Rev_supp.unit = "كبرى";
                }
                    db.tb_rev_supp.Add(tb_Rev_supp);
                db.SaveChanges();


                // tb_Supp.total_price = Convert.ToDouble(txt_stay.Text) ;

                tb_supp.total_price = Convert.ToDouble(text_stay.Text);
                ...
                string notificationMessage = $"لقد قام :  {frm_supp_Add.la_username.Text}   بارجاع مشتريات الى المورد : {tb_Rev_supp.name_supp} اسم الصنف : {tb_Rev_supp.name_type}  ";
                ...
                toast.txt_caption.Text = "تم اضافة مرتجعات الى المورد";
                toast.Show();
                this.Close();
        ...
        private void small_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (small.Checked)
            {
                ...
                else
                {
                    ...
                    tb_Stuk = db.tb_stuk.Where(x => x.stuk_name == edt_type.Text).FirstOrDefault();
                    edt_price.Text = tb_Stuk.stuk_buy.ToString();
                    big.Checked = false;
                }
            }
        }
        ...
        private void big_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                tb_Stuk = db.tb_stuk.Where(x => x.stuk_name == edt_type.Text).FirstOrDefault();
                edt_price.Text = (tb_Stuk.stuk_buy * tb_Stuk.stuk_qtfacttotal).ToString();
                small.Checked = false;
            }
        }
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    tb_Rev_supp = db.tb_rev_supp.Where(x => x.id == id).FirstOrDefault();
                    db.Entry(tb_Rev_supp).State = EntityState.Deleted;
                    db.SaveChanges();
                    ...
                    if (tb_Rev_supp.unit == "صغرى")
                    {
                        tb_Stuk = db.tb_stuk.Where(x => x.stuk_name == tb_Rev_supp.name_type).FirstOrDefault();
                        tb_Stuk.stuk_qt += tb_Rev_supp.qt;
                        db.Entry(tb_Stuk).State = System.Data.Entity.EntityState.Modified;
                        db.SaveChanges();
                    }
                    else if (tb_Rev_supp.unit == "كبرى")
                    {
                        tb_Stuk = db.tb_stuk.Where(x => x.stuk_name == tb_Rev_supp.name_type).FirstOrDefault();
                        rr= Convert.ToDouble(tb_Rev_supp.qt )* Convert.ToDouble(tb_Stuk.stuk_qtfacttotal);
                        tb_Stuk.stuk_qt += rr;
                        db.Entry(tb_Stuk).State = System.Data.Entity.EntityState.Modified;
                        db.SaveChanges();
                    }
                    ...
                    main frm_supp_Add = (main)Application.OpenForms["main"];
                    string notificationMessage = $"لقد قام :  {frm_supp_Add.la_username.Text}   بحذف عملية ارجاع مشتريات : {tb_Rev_supp.name_supp} اسم الصنف : {tb_Rev_supp.name_type}  ";
                    send.SendEmailWithHtmlGridData(notificationMessage);
                    notfication.notfication1 = notificationMessage;
                    notfication.CreatedAt = DateTime.Now;
                    ...
                    tb_supp = db.tb_supp.Where(x => x.supp_name == tb_Rev_supp.name_supp).FirstOrDefault();
                    tb_supp.total_price +=tb_Rev_supp.price;
